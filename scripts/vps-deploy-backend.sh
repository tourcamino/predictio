...
if command -v node >/dev/null 2>&1 && [[ -f src/server/scripts/runGlobalPaperSettlementTick.ts ]]; then
  ...
  # Host-run: postgres is published on 127.0.0.1:5432 (see docker-compose.prod.yml).
  if [[ -x scripts/vps-run-settlement-tick.sh ]]; then
    scripts/vps-run-settlement-tick.sh || echo "WARN: settlement tick failed (non-fatal)"
  else
    echo "WARN: scripts/vps-run-settlement-tick.sh missing — skip settlement tick"
  fi
fi
