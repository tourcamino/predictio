...
/** Min share of total budget per OPEN slot (prevents dust when many markets). */
export const MIN_ALLOCATION_SHARE = 0.06;
/** Max share per slot — keeps 4-market sets from over-concentrating on one fixture. */
...
/**
 * Split `totalBudget` across slots by normalized appeal/volume weights with min/max share clamps.
 * Sum of allocations equals `totalBudget` (±0.01 USDC).
 */
