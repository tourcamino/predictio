...
function SomeComponent() {
  ...
  // or, for a streaming query, we can await the query to get an async iterator:

  async function getStreamingQuery() {
    const result = await trpcClient.someStream.query({ name: "Jerry" });
